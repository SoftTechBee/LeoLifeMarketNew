...
public partial class Admin_Accountmaster : System.Web.UI.Page
{
    ...
    protected void Button1_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            {
                string sql = "insert into tblcoupanwallet(username,debit,DOI,type,remark)values('" + txtusername.Text+"','"+txtAmount.Text+"','"+ objtime.returnStringServerMachTime()+ "','Admin','" + txtremarks.Text+"')";
            ...
            {
                string sql = "insert into tblcoupanwallet(username,Credit,DOI,type,remark)values('" + txtusername.Text + "','" + txtAmount.Text + "','" + objtime.returnStringServerMachTime() + "','Admin','" + txtremarks.Text + "')";
                int a = objcon.ExecuteSqlQuery(sql);
                if (a > 0)
                {
                    lbsuccess.Text = "Transaction  Successed";
